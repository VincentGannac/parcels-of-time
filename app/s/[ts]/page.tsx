// app/s/[ts]/page.tsx — Public page for a claimed second
import { formatISOAsNice } from '@/lib/date'
import { absoluteUrl } from '@/lib/url'

type Params = { ts: string }

async function getSecond(ts: string) {
  const url = await absoluteUrl(`/api/seconds/${encodeURIComponent(ts)}`) // 👈 await
  const res = await fetch(url, { cache: 'no-store' })
  if (!res.ok) return { claimed:false }
  return res.json()
}

// En Next 15, params est un Promise
export default async function Page({ params }: { params: Promise<Params> }) {
  const { ts } = await params
  const decodedTs = decodeURIComponent(ts)
  const data = await getSecond(decodedTs)
  ...
  return (
    <main style={{fontFamily:'Inter, system-ui', background:'#FAF9F7', color:'#0B0B0C', minHeight:'100vh'}}>
      <section style={{maxWidth:860, margin:'0 auto', padding:'72px 24px'}}>
        ...
        <h1 style={{fontFamily:'Space Grotesk, Inter, system-ui', fontSize:40, margin:'16px 0 4px'}}>Second</h1>
