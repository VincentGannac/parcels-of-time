...
type RegistryItem = {
  id: string
  ts: string            // ISO minute en UTC (ex. "2024-06-01T11:11:00Z")
  title?: string | null
  message?: string | null
}
      ...
      <section style={{position:'relative', borderBottom:'1px solid var(--color-border)'}}>
        ...
        <div style={{maxWidth:1280, margin:'0 auto', padding:'48px 24px 28px'}}>
          ...
          <p style={{maxWidth:860, fontSize:18, lineHeight:'28px', opacity:.95, margin:0}}>
            Une œuvre collective, anonyme, dédiée aux <strong>réussites</strong> et à <strong>l’amour</strong>.  
            Ici, seuls les éléments rendus publics par les propriétaires sont affichés : <em>minute UTC</em>, <em>ID</em>, <em>titre</em> et <em>message</em>.
            Faites défiler — laissez ces fragments former un <strong>chœur de moments</strong>.
          </p>
        </div>
      </section>

      {/* Client (recherche, filtres, infinite scroll, effet “galerie”) */}
      <section style={{maxWidth:1280, margin:'0 auto', padding:'20px 24px 56px'}}>
        <RegistryClient initial={initial} apiHref="/api/registry" />
      </section>
      ...
      <footer style={{borderTop:'1px solid var(--color-border)', color:'var(--color-muted)'}}>
        <div style={{maxWidth:1280, margin:'0 auto', padding:'16px 24px', display:'flex', justifyContent:'space-between', flexWrap:'wrap', gap:10}}>
          ...
          <div style={{display:'flex', gap:14}}>
            <a href="/claim" style={{textDecoration:'none', color:'inherit'}}>Réserver une minute</a>
            <a href="/claim?gift=1" style={{textDecoration:'none', color:'inherit'}}>Offrir</a>
          </div>
        </div>
      </footer>
    </main>
