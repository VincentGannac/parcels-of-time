...
export default async function LoginPage({
  ...
  const labels =
    locale === 'fr'
      ? { title: 'Connexion', email: 'Adresse email', password: 'Mot de passe', submit: 'Se connecter', switch: "Pas de compte ? S'inscrire →" }
      : { title: 'Sign in',   email: 'Email address', password: 'Password', submit: 'Sign in',           switch: 'No account? Sign up →' }

  const errKey = sp?.err
  const errText =
    errKey === 'missing' ? (locale === 'fr' ? 'Email et mot de passe requis.' : 'Email and password are required.')
    : errKey === 'badcreds' ? (locale === 'fr' ? 'Identifiants invalides. Vérifiez votre email et votre mot de passe.' : 'Invalid credentials. Check your email and password.')
    : errKey === 'server' ? (locale === 'fr' ? 'Erreur serveur. Réessayez dans un instant.' : 'Server error. Please try again.')
    : null
  ...
  return (
    <main style={{ maxWidth: 520, margin: '0 auto', padding: '32px 20px', fontFamily: 'Inter, system-ui' }}>
      ...
      {sp?.next && (
        <div style={{ margin: '0 0 12px', padding: '10px 12px', background: '#fff3bf', border: '1px solid #ffe08a', borderRadius: 10, fontSize: 13, color: '#6b4e00', fontWeight: 700 }}>
          Vous vous connectez pour accéder à : <code style={{ fontWeight: 800 }}>{sp.next}</code>
        </div>
      )}

      {errText && (
        <div role="alert" aria-live="assertive" style={{ marginBottom: 8, padding: '12px 14px', borderRadius: 10, border: '1px solid #f19999', background: '#ffecec', color: '#8a1f1f', fontWeight: 600 }}>
          {errText}
        </div>
      )}
      ...
      {sp?.info === 'magic_disabled' && (
        <p style={{ background: '#eef7ff', border: '1px solid #cbe4ff', padding: 10, borderRadius: 8 }}>
          {locale === 'fr' ? "La connexion par lien magique n'est plus disponible. Connectez-vous avec votre mot de passe." : 'Magic link sign-in is disabled. Please sign in with your password.'}
        </p>
      )}

      {/* action relative => évite tout rebond d’hôte */}
      <form action={`/api/auth/login`} method="post" style={{ display: 'grid', gap: 12, marginTop: 12 }}>
        ...
        <label style={{ display: 'grid', gap: 6 }}>
          ...
          <input name="password" type="password" required autoComplete="current-password" style={{ padding: '10px 12px', border: '1px solid #ddd', borderRadius: 8 }} />
        </label>
