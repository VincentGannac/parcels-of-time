...
// ── Contenu principal de la page (utilise useSearchParams) ─────────────────────
function ClaimFormInner() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      ...
      setStatus('ok');
      router.push(`/s/${encodeURIComponent(data.ts)}`);
      return;
    }
    if (res.status === 409) {
      setStatus('conflict');
      setError('This second is already claimed. Try another.');
      return;
    }
    setStatus('error');
    try {
  ...
  return (
    ...
    >
      <section style={{ maxWidth: 720, margin: '0 auto', padding: '64px 24px' }}>
        ...
        <h1 style={{ fontFamily: 'Space Grotesk, Inter, system-ui', fontSize: 40, margin: '16px 0' }}>
          Claim your second (Phase A)
        </h1>
        <p style={{ opacity: 0.8 }}>
          This MVP lets you claim a second for free to test the flow. Payment comes in Phase B.
        </p>
        ...
        <form onSubmit={onSubmit} style={{ display: 'grid', gap: 12, marginTop: 12 }}>
          ...
          <label style={{ display: 'grid', gap: 6 }}>
            ...
            <input
              ...
              style={{ padding: '12px 14px', border: '1px solid #D9D7D3', borderRadius: 8 }}
            />
            <span style={{ opacity: 0.7, fontSize: 12 }}>Or pick local time:</span>
            <input
              type="datetime-local"
              onChange={(e) => setForm((f) => ({ ...f, ts: e.target.value }))}
              style={{ padding: '12px 14px', border: '1px solid #D9D7D3', borderRadius: 8 }}
...
// ── Export par défaut : wrapper Suspense (fix Next prerender) ───────────────────
export default function Page() {
  return (
    <Suspense fallback={<main style={{ padding: 32, fontFamily: 'Inter, system-ui' }}>Loading…</main>}>
      <ClaimFormInner />
    </Suspense>
  );
}
