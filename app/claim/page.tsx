...
function safeDecode(value: string): string {
  // Décode 1..n fois si besoin (gère %3A et %253A, etc.)
  let out = value
  try {
    // Décode jusqu’à stabilisation, en évitant les boucles
    for (let i = 0; i < 3; i++) {
    ...
    // ignore si ce n'est pas encodable
  }
  return out
}
...
export default function Page() {
  ...
  const [form, setForm] = useState({
    ...
    message: '',
    link_url: '',
    ts: prefillTs, // pré-rempli décodé
  })
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    // 2) Essaie de construire un Date valide puis normalise en ISO Z à la seconde
    ...
    // 3) Envoie à l’API Checkout (Stripe)
