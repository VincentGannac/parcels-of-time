...
function labels(loc: 'fr' | 'en'): L {
  return loc === 'fr'
    ? {
        ...
        currency: 'Devise',
        mpNote:
          'Vente opérée via Parcels of Time (Stripe Connect). En cas de vente entre particuliers, ce document fait office de reçu.',
    ...
    : {
        ...
        currency: 'Currency',
        mpNote:
          'Sale operated via Parcels of Time (Stripe Connect). For peer-to-peer sales, this document acts as a receipt.',
        platform: process.env.INVOICE_ISSUER_NAME || 'Parcels of Time',
      }
}

/** Robust YYYY-MM-DD → ISO midnight */
function toIsoDay(ymd: string): string | null {
  try {
    const d =
      /^\d{4}-\d{2}-\d{2}$/.test(ymd) ? new Date(`${ymd}T00:00:00.000Z`) : new Date(ymd)
...
export async function GET(req: Request, ctx: RouteCtx) {
  ...
  const buf = await pdfBuffer((doc: any) => {
    ...
    doc.moveDown(0.5)
    doc.text(t.buyer, 320, topY, { underline: true })
    doc.moveDown(0.3)
