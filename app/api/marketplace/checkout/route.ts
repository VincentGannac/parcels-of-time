...
export async function POST(req: Request) {
  const base = process.env.NEXT_PUBLIC_BASE_URL || new URL(req.url).origin
  if (!process.env.STRIPE_SECRET_KEY) return jsonError('stripe_key_missing', 500)
  ...
  try {
    ...
    if (isForm) {
      ...
      locale         = (String(form.get('locale') || '') === 'en') ? 'en' : 'fr'
    ...
    if (!buyerEmail) return isForm
      ? NextResponse.redirect(`${base}/account?err=missing_buyer_email`, { status: 303 })
      : jsonError('missing_buyer_email', 400)
    ...
    return isForm
      ? NextResponse.redirect(`${base}/account?err=stripe_error`, { status: 303 })
      : jsonError('stripe_error', 500, { detail: String(err?.message || err) })
  }
}
