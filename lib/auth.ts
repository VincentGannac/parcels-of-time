...
function sign(input: string) {
  // Signature HMAC-SHA256 encodée en Base64URL
  return b64url(crypto.createHmac('sha256', SECRET).update(input).digest())
}

// Accepte Base64URL et Base64 "classique", puis renvoie l'UTF-8
...
/** Construit la valeur du cookie (payload.base64 + '.' + signature) */
export function encodeSessionForCookie(sess: Session): string {
  const payload = Buffer.from(JSON.stringify(sess)).toString('base64') // peut contenir '='
  const sig = sign(payload) // signature sur le payload EXACT
  return `${payload}.${sig}`
}
...
/* =========================================
 *  Lecture/écriture côté "server components"
 * ========================================= */
...
export async function readSession(): Promise<Session | null> {
  ...
  // Certains environnements percent-encodent les '=' du base64 (→ %3D)
  try { raw = decodeURIComponent(raw) } catch { /* no-op */ }

  const [p, s] = raw.split('.')
  if (!p || !s) return null

  // La signature est calculée sur la chaîne payload EXACTE (non découpée/transformée)
  if (sign(p) !== s) return null

  try {
    const json = b64anyToUtf8(p) // tolère base64url ET base64 classique
...
/** Variante "server-only" (pas utilisée dans les routes API, mais pratique ailleurs) */
